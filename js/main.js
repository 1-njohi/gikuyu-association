  ...
  if (form) {
    ...
    form.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        console.log(
          "📨 Fetch completed (no-cors means we can't read response)"
        );
        showMessage(
          "🎉 Registration successful! Welcome to Nyũmba ya Mũmbi.",
          "success"
        );
        form.reset();
      } catch (error) {
        ...
        showMessage(
          "⚠️ Something went wrong. Please try again or contact the association directly.",
          "error"
        );
      } finally {
        submitBtn.disabled = false;
...
// ==========================================
// ZOOM OVERLAY FUNCTIONS
// ==========================================

// Open the zoom overlay with the clicked image
...
// Close zoom if user clicks on the overlay background (not the image)
document.addEventListener("DOMContentLoaded", function () {
  const overlay = document.getElementById("zoom-overlay");
  if (overlay) {
